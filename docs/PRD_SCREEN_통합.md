...
| F9 | **경력공고 비교 (성장 로드맵)** | 같은 회사·직무의 경력직 공고 대비, 신입 입사 후 단계적으로 필요해지는 역량 시각화(점수화 안 함) | 진단 결과(analysis, posting) | 성장 로드맵 섹션 | ✅ 구현 완료 |
| F10 | 관심 기업 북마크 · 트래킹 | 기업 저장, 경험 추가 시 적합도 변화 추적 | - | - | ⏭ 장기 (계정 필요) |
| F11 | 갭 보완 활동 추천 | 부족 역량을 채울 국비지원(내일배움카드) 강의 추천 | F9의 향후 필요 역량 | 강의 리스트 | ✅ 구현 완료(강의 추천만. 프로젝트·자격증 등 비교과 활동 추천은 미착수) |

**MVP 경계**: F4가 *"무엇이 비었는지"* 까지 보여주고, *"그래서 뭘 하라"* 는 F9·F11이다. 진단과 처방을 분리한다.

F9·F11은 원래 "⏭ 확장/장기"로 로드맵 뒤쪽에 있었으나, 2026-08-26 사용자 요청으로
F4 직후 구현했다. 자세한 계약은 `docs/API.md`의 `POST /api/career/outlook`,
데이터 구조는 `docs/공고데이터/chafit_01_스키마_분류체계.md` §6-1-1 참고.
...
| 구현 완료 | **경력공고 비교 + 강의 추천 (F9, F11)** | 원래 확장/장기였으나 2026-08-26 조기 구현 |
| 배포 후 | **공고 탐색 오픈API 연동 (F7)** — 워크넷/공공데이터포털, Adzuna | 공고를 직접 찾아 F3에 투입 |
| 확장 | 자소서 문항 매핑 (F8) | 발표 임팩트 강화 |
| 장기 | 북마크 · 트래킹 (F10), 갭 리포트 → 비교과 활동(프로젝트·자격증 등) 추천 확대 | 계정 기능 필요 |
