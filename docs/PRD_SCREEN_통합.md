...
기존 채용 플랫폼은 (1)에 필요한 검색 · 필터는 잘 제공하지만, (2)에 대해서는 아무것도 해주지 않는다. 공고에는 *"RESTful API 설계 및 운영 경험"* 이라고 적혀 있고, 이력서에는 *"졸업작품에서 서버를 만들어 봤다"* 고 적혀 있다. **이 둘이 같은 것인지 다른 것인지를 판단해 주는 서비스가 없다.** 그래서 지원자는 확신 없이 지원하거나, 확신이 없어서 지원하지 않는다.
...
| 항목 | 내용 |
|---|---|
| 상황 | 4년제 컴퓨터공학 졸업예정. 백엔드 직군으로 방향은 확정했고, 지원할 회사도 몇 곳 추려 놓음 |
| 보유 경험 | 캡스톤 팀프로젝트(웹 서비스 백엔드), 데이터분석 동아리 2년, 스타트업 인턴 2개월, 정보처리기사 · SQLD |
| 현재 해결 방식 | 공고를 열어 자격요건을 눈으로 훑고 "이 정도면 되나?" 자문 → 결론이 안 나서 지원을 미룸 |
| 핵심 Pain | ① 공고의 *"RESTful API 설계 및 운영 경험"* 과 자기 경험 *"졸업작품에서 서버 만들어 봄"* 을 연결하지 못한다. ② 어떤 항목이 충족이고 어떤 항목이 미충족인지 **분해가 안 된다.** |
| 니즈 | "이 공고 기준으로 내가 몇 %쯤 되는지, 그리고 **무엇 때문에** 그런지 알고 싶다" |
| Aha 모먼트 | 자기 캡스톤 경험이 **공고의 특정 요구사항과 직접 연결되어** 근거 문장으로 제시될 때 |
| 대표 인용 | *"제 캡스톤 프로젝트가 이 공고 기준으로 무슨 값어치인지 아무도 안 알려줘요."* |

**A의 이용 시나리오**
1. 이력서 PDF를 업로드한다.
2. 지원하려는 백엔드 공고를 붙여넣는다.
3. 진단서에서 **충족 역량**(Java, Spring Boot, MySQL)과 **필요 역량**(Docker, AWS)이 분리되어 보인다.
4. 근거 문장을 읽는다 — *"캡스톤의 팀 서버 개발 경험이 요구 역량 중 'RESTful API 설계'와 직접 대응합니다."* → **여기가 데모의 하이라이트.**
5. 다른 회사 공고를 하나 더 넣어 비교한다.
...
| 페르소나 | 샘플 이력서 | 샘플 공고 |
|---|---|---|
| A (주 시연) | 컴퓨터공학 졸업예정자 이력서 1부 | 백엔드 개발자 공고 2~3건 |
| B (보조 시연) | 제조 QA 3년차 경력기술서 1부 | 데이터 분석가 공고 2~3건 |
...
- **`experiences`가 핵심이다.** F4의 근거 문장이 *"캡스톤의 팀 서버 개발 경험이 …"* 처럼 **구체적 경험을 지목**하려면, 스킬 태그만으로는 부족하고 경험 단위 서술이 필요하다.
