...
```yaml
jd_id: KR-IND01-JOB02-001        # 국가-산업-직무-일련번호. 경력 페어는 여기에 X를 붙인다
pair_id: KR-IND01-JOB02-001      # 신입↔경력 페어를 묶는 공통 키. 항상 신입 쪽 jd_id 값
...
posting:
  ...
  experience_years: null         # 경력 항목만 채움. 예: "4~9년"
  hiring_type: 정기공채
  education_min: 학사
...
| `pair_id` | 신입↔경력을 회사명·직무명 추론이 아니라 **명시적 키**로 묶는다. 회사가 코퍼스에 여러 직무로 등장할 때 오매칭을 막는다 | F9(경쟁력 비교) |
...
| 계층 | 대상 | 방법 | 커버리지 |
|---|---|---|---|
| L1 시드 | 대기업·중견기업 주요 직무 | 본 코퍼스 **86건**(신입) 수기 정형화 | 데모 시연에 충분 |
...
### 6-1-1. 신입×경력 페어링 (F9 경쟁력 비교)

L1 시드 86건 각각에, **같은 회사·같은 직무의 경력직 버전**을 `pair_id`로 묶어 추가했다
(`web/src/lib/jobs/fixtures/jd-corpus.json`, 총 172건). 실시간 크롤링이 아니라 L1과
동일한 방법론(표준 템플릿, `verification: C`)으로 만들었다 — §7의 크롤링 금지 원칙을
그대로 지킨다.

경력 항목의 `competency_tags`는 신입 항목보다 1~3개 많다. 이 차집합
(`career.competency_tags − entry.competency_tags − 사용자_보유_태그`)이 "입사 후
단계적으로 필요해지는 역량"의 재료다 — 점수화하지 않고 목록으로만 보여준다
(§9.1 성장 로드맵 섹션, 절대 "부족하다"로 표현하지 않는다).

생성 스크립트는 `scripts/build_corpus.py`에 있다. 86건을 한 번에 새로 쓰는 대신,
회사별로 "추가 책임·자격·태그" 델타만 입력하면 공통 필드(회사·전공·지역 등)는
기계적으로 채워 넣는 방식이라, 나중에 시드를 늘릴 때도 같은 패턴을 재사용할 수 있다.
